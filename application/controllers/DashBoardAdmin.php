	...
	function export(){
		...
		//No Filter
			...
			if($freq == 1){
				...
				if($type == 'tbleducation'){
					$file_name = '[MASTER]'.' Student_Education_Information_'.date('m').'-'.date('d').'-'.date('Y').'.csv'; 
					header("Content-Description: File Transfer"); 
					header("Content-Disposition: attachment; filename=$file_name"); 
					header("Content-Type: application/csv;");
					$student_data = $this->Report_model->fetch_Education();
					$file = fopen('php://output', 'w');
					$header = array("Entry#","Student#","Elementary Name","Elem. Year","Elem. Honors","Jr. High Name","JrH Year","JrH Honors","Sr. High Name","SrH Year","SrH Honors","College Name","College Year Last Attended","College Honors","Date Entry"); 
					fputcsv($file, $header);
					foreach ($student_data->result_array() as $key => $value){ 
						fputcsv($file, $value); 
					}
